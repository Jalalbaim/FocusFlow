...
# -----------------------------
# 1) Forward noising for FlowMatch (you@torch.no_grad()
...
# -----------------------------
# 2) Import the functions you already have
# -----------------------------
# Paste here the full code I gave you:
# - scale_noise
# - calc_v_sd3
# - _encode_prompts_sd3
# - _prep_mask_for_latents
# - _clip_extremes_percentile
# - _soften_mask
# - create_diffedit_mask_sd3
# - FlowEditSD3_masked
#
# (I'm not repeating them here to keep this runnable example short.)
...
def _clip_extremes_percentile(x: torch.Tensor, q_low: float = 0.01, q_high: float = 0.99) -> torch.Tensor:
    """
    DiffEdit says "remove extreme values" but does not specify the exact rule. :contentReference[oaicite:3]{index=3}
...
def create_diffedit_mask_sd3(
    ...
    T_steps: int = 50,
    strength: float = 0.5,        # DiffEdit default: 50% noise :contentReference[oaicite:4]{index=4}
    n: int = 10,                  # DiffEdit default n=10 :contentReference[oaicite:5]{index=5}
    ...
    # "remove extreme values" (explicit percentile clipping; exact rule not specified in paper) :contentReference[oaicite:8]{index=8}
    mask = _clip_extremes_percentile(mask, q_low=q_low, q_high=q_high)

    # rescale to [0,1] (DiffEdit) :contentReference[oaicite:9]{index=9}
...
@torch.no_grad()
def encode_image_to_latents(pipe, image: Image.Image, size=1024):
    x = preprocess_pil(image, size=size).to(pipe.device, dtype=pipe.vae.dtype)
    latents_dist = pipe.vae.encode(x).latent_dist
    latents = latents_dist.sample()
...
if mask_soft is not None:
    ...
    plt.subplot(1,3,1); plt.title("Input"); plt.imshow(init_arr); plt.axis("off")
    plt.subplot(1,3,2); plt.title("Mask (soft)"); plt.imshow(m, cmap="gray"); plt.axis("off")
